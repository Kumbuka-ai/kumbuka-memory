...
/**
 * The lock closes, and the data is still there.
 *
...
class FailClosedProbeIT {
    ...
    /** The scope the probe entries sit in. Any uuid; fixed for legibility. */
    ...
    void an_unbound_read_returns_nothing_and_the_rows_are_still_there() throws SQLException {
        ...
        try (Connection c = Db.asService()) {
            ...
            assertThat(Db.countEntriesKeyed(c, key))
                .as("a transaction that never bound a tenant must see nothing at all — "
                    + "the predicate fails closed, and this is the half that is the "
                    + "guarantee")
                .isZero();
            ...
            Db.bindTenant(c, tenantA);
            assertThat(Db.countEntriesKeyed(c, key))
                .as("and with the tenant bound the entry is present and unchanged — which "
                    + "is what makes the emptiness above a lock rather than an absence")
                .isEqualTo(1);
        }
    }
     ...
     * caller believing it had recorded an entry.
     */
    @Test
    void an_unbound_write_is_refused_by_the_policy() throws SQLException {
        try (Connection c = Db.asService()) {
            Db.bindTenant(c, null);
            try {
                Db.insertEntry(c, tenantA, "written-with-nothing-bound");
    ...
    void the_orm_filter_isolates_on_its_own_when_the_policy_is_off() throws Exception {
        ...
        try (Connection owner = Db.asMigrator()) {
            try {
                ...
                try (Connection c = Db.asService()) {
                    Db.bindTenant(c, tenantA);
                    assertThat(Db.countEntriesKeyed(c, keyB()))
                        .as("RED STATE, observed: with row-level security off, raw SQL "
                            + "under tenant A reads tenant B's entry too. The ORM filter "
                            + "cannot reach a statement it did not build, which is the "
                            + "whole reason for a second layer")
                        .isEqualTo(1);
                }
            } finally {
                Db.exec(owner, "ALTER TABLE " + Db.ENTRY_TABLE + " ENABLE ROW LEVEL SECURITY");
                owner.commit();
            }
        }
    }
    ...
    void the_policy_isolates_raw_sql_that_the_orm_never_touched() throws Exception {
        ...
        try (Connection c = Db.asService()) {
            Db.bindTenant(c, tenantA);
            assertThat(Db.countEntriesKeyed(c, keyB()))
                .as("raw SQL bypasses the ORM filter entirely, so this count is the "
                    + "policy's work and nobody else's")
                .isZero();
            assertThat(Db.countEntriesKeyed(c, keyA()))
                .as("and its own entry is there, so the zero above is a filter rather than "
                    + "an empty table")
                .isEqualTo(1);
        }
    }
    ...
    /**
     * One entry per tenant, written through the ORM so both layers are on the
     * path.
     *
     * <p>The keys carry the tenant id. Two cases below plant a pair each, and
     * one of them then reads with row-level security switched OFF — where raw
     * SQL sees the whole table, the other case's rows included. Fixed keys
     * would make that count depend on execution order, and a failure would
     * read as a broken policy rather than as two tests sharing a name.
     */
    private void plantOneEntryPerTenant() throws Exception {
        try (AutoCloseable ignored = tenantContext.bind(tenantA)) {
            orm.write(SCOPE, keyA());
        }
        try (AutoCloseable ignored = tenantContext.bind(tenantB)) {
            orm.write(SCOPE, keyB());
        }
    }
