...
/**
 * This service's role holds its own schema, plus {@code SELECT} on exactly
 * one view, and nothing else anywhere.
 *
 ...
 * catalogue, so it also covers the neighbour that does not exist yet and the
 * grant somebody issues next year.
 *
 * <h2>The permitted set is a list of objects, not of schemas</h2>
 *
 * The service consumes a platform read contract, so it holds something
 * outside its own schema. Adding {@code platform} to a list of permitted
 * SCHEMAS would have been the obvious edit and would have been a blank
 * cheque: every future object in that schema would pass unnoticed, including
 * one granted by mistake. What is permitted is one view and one privilege on
 * it, so that is what the list holds — and
...
class ServiceRoleConformanceIT {
     ...
     */
    private static final String OWN_SCHEMA = "memory";

    /**
     * Everything the role may hold OUTSIDE its own schema, named object by
     * object. One entry: the published read contract.
     */
     ...
     * is seeing. A permitted entry that is not actually held would make the
     * check pass by absence.
     */
    @Test
    void the_permitted_grant_is_really_the_one_being_permitted() throws SQLException {
        assertThat(grantsOn(SubstrateDatabaseResource.PLATFORM_SCHEMA,
            ...
            .as("the role must actually hold SELECT on the read contract, and only SELECT "
                + "— the contract is a question the service asks, never one it answers")
     ...
     *
     * <p>It owns nothing anywhere — that is asserted in
     * {@link ServiceRolePrivilegeIT} and it is the point of the whole
     * arrangement — so the question has to be asked of the grants instead.
     * Without it, a role stripped of every privilege would satisfy each
     * assertion above: a green suite describing a deployment that cannot
     * start.
     */
    @Test
    void the_service_role_does_hold_grants_in_its_own_schema() throws SQLException {
        assertThat(grantsOn(OWN_SCHEMA, "memory"))
            .as("the enumerated entitlement of V2 must actually be in place on the entry "
                + "table")
            .containsExactly("DELETE", "INSERT", "SELECT", "UPDATE");
        assertThat(grantsOn(OWN_SCHEMA, "content_relation"))
